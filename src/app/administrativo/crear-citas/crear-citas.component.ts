...
export class CrearCitasComponent {
  ...
  siguiente() {
    ...
    if (this.formularioCita.valid) {
      ...
      this.formularioCita.markAllAsTouched(); // Marca todos los campos como tocados para mostrar los mensajes de error
    }
  }
  ...
  crearCita() {
    ...
    if (this.formularioCita.valid) {
      ...
      console.log(
        'Horas Disponibles:',
        fechaFormateada + ' ' + horasDisponibles + ':00'
      );
      ...
      this.http
        .post<any>('http://localhost/api/v1/crear-citas', crear)
        .subscribe(
          (response) => {
